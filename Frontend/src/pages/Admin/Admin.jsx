...
export default function Admin() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('http://localhost:5001/api/fetch/review');
        if (!response.ok) {
  ...
  const handleAccept = async (Id, Status) => {
    try {
      // Delete API
      const deleteResponse = await fetch(`http://localhost:5001/api/delete/review/${Id}`, {
        method: 'DELETE',
      });
      ...
      // Update Status API
      const statusResponse = await fetch('http://localhost:5001/api/update/status', {
        method: 'PUT',
        headers: {
      ...
      if (!statusResponse.ok) {
        throw new Error(`Failed to update status for record with id ${Id}`);
      }
  ...
  const handleAddUser = async () => {
    ...
    try {
      const response = await fetch("http://localhost:5001/api/add/user", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const handleRemoveUser = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:5001/api/delete/user/${username}`, {
        method: "DELETE",
      });
          ...
          <div className="ad-content">
            ...
            <div className="ad-card ad-card-2">
              ...
              <div className="ad-content-card-box-3">
                <button className="ad-button-blue" type="button" onClick={handleAddUser}>
                  Add
                </button>
                <button className="ad-button-red" type="button" onClick={handleRemoveUser}>
                  Remove
                </button>
              </div>
            </div>
                      ...
                      {currentRecords.length > 0 ? (
                        currentRecords.map((record) => (
                          <tr key={record.Id}>
                            ...
                            <td>{record.Status}</td>
                            <td>
                              <button type="button" onClick={() => handleAccept(record.Id, record.Status)}>✔</button>
                              <button type="button" onClick={() => handleReject(record.Id)}>✘</button>
                            </td>
                          </tr>
                        ))
                      ) : (
